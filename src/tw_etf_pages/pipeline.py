...
from .parse import parse_excel_for_issuer, parse_uni_excel, parse_fh_excel, parse_zdsetf_snapshot, parse_yuanta_ratio, parse_cathay_weights, parse_uobam_pcf, parse_capital_buyback
...
def _parse_fetched(
    ...
    if kind == "uobam":
        return parse_uobam_pcf(path, etf.ticker, cfg.placeholder)
    if kind == "capital":
        return parse_capital_buyback(path, etf.ticker, cfg.placeholder)
...
def _sample_map(samples: Path) -> dict[str, list[Path]]:
    ...
    mapping: dict[str, list[Path]] = {
        ...
        "00939": [samples / "00939_20260924.xlsx"],
    }
    return mapping
