"""Minimal tests: config loads 00401A; parse_zdsetf on saved jpm sample."""
...
def test_config_loads_00401A_jpm():
    cfg = load_config(ROOT)
    etf = cfg.etf("00401A")
    assert etf.name == "主動摩根台灣鑫收"
    assert etf.issuer == "jpm"
    assert etf.fund_code == "00401A"
    assert "am.jpmorgan.com" in etf.source_page
    assert "00401a1" in etf.source_page.lower() or "00401A" in etf.source_page


def test_parse_zdsetf_00401A_sample():
    cfg = load_config(ROOT)
    path = ROOT / "samples" / "zdsetf_00401A_snapshot.json"
    assert path.is_file(), "missing samples/zdsetf_00401A_snapshot.json"
    snap = parse_zdsetf_snapshot(path, "00401A", cfg.placeholder)
    assert snap["etf_ticker"] == "00401A"
    ...
    # (00401A sample has all-null weights → 0.0; do not invent non-zero weights)
    assert all(isinstance(h["weight_pct"], float) for h in snap["holdings"])
    assert all(h["weight_pct"] == 0.0 for h in snap["holdings"])
    meta = snap.get("meta") or {}
    assert "am.jpmorgan.com" in str(meta.get("source_url") or "")


def test_parse_zdsetf_aliases_still_work_for_prior_issuers():
    """Alias hardening must not break capital / fubon / ctbc / nomura / allianz / cathay samples."""
    cfg = load_config(ROOT)
...
if __name__ == "__main__":
    test_config_loads_00401A_jpm()
    test_parse_zdsetf_00401A_sample()
