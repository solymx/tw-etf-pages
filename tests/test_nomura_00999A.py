"""Minimal tests: config loads 00999A; parse_zdsetf on saved nomura sample."""
...
def test_config_loads_00999A_nomura():
    cfg = load_config(ROOT)
    etf = cfg.etf("00999A")
    assert etf.name == "主動野村臺灣高息"
    assert etf.issuer == "nomura"
    assert etf.fund_code == "00999A"
    assert "nomurafunds.com.tw" in etf.source_page
    assert "00999A" in etf.source_page


def test_parse_zdsetf_00999A_sample():
    cfg = load_config(ROOT)
    path = ROOT / "samples" / "zdsetf_00999A_snapshot.json"
    assert path.is_file(), "missing samples/zdsetf_00999A_snapshot.json"
    snap = parse_zdsetf_snapshot(path, "00999A", cfg.placeholder)
    assert snap["etf_ticker"] == "00999A"
...
def test_parse_zdsetf_aliases_still_work_for_prior_nomura():
    """Adding 00999A must not break 00980A / 00985A sample parse."""
    cfg = load_config(ROOT)
    for ticker in ("00980A", "00985A"):
...
if __name__ == "__main__":
    test_config_loads_00999A_nomura()
    test_parse_zdsetf_00999A_sample()
